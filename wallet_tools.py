...
@wallet_tools.command('EthBalanceToPrometheus')
@click.argument('eth_address', required=True)
@click.argument('prometheus_import_url', required=False)
def EthBalanceToPrometheus(eth_address, prometheus_import_url):
    from lib.balance import push_eth_balance_to_prometheus
    push_eth_balance_to_prometheus(eth_address, prometheus_import_url)
