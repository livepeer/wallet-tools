...
def push_eth_balance_to_prometheus(address: str, prometheus_url: str) -> None:
    address_normalised = w3.to_checksum_address(address)
    balance_wei = w3.eth.get_balance(address_normalised)
    print(f"Balance for {address_normalised}: {balance_wei} wei")
    registry = CollectorRegistry()
    labels = {"address": address_normalised}
    g = Gauge('livepeer_wallet_balance_wei', "ethereum wallet balance in wei", labels.keys(), unit="wei", registry=registry)
    g.labels(**labels).set(balance_wei)
    push_to_gateway(prometheus_url, job='wallet-tools', registry=registry)
